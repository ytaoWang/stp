...
static int __do_btree_split_leaf(struct stp_btree_info *sb,struct stp_bnode *root,struct stp_bnode *node,int pos)
{
  ...
  //  if(!node->parent) 
  {
      ...
      if(!node->item->parent.offset && !is_root(sb,root)) {
          assert(root->parent);
...
static  int __do_btree_split_internal(struct stp_btree_info *sb,struct stp_bnode *root,struct stp_bnode *node,int pos)
{
  ...
  while(i < BTREE_CHILD_MAX_TEST) {
    ...
    __read_ptrs(root,i);
    if(root->ptrs[i]) {
        __set_header(&root->ptrs[i]->item->parent,&_new->item->location);
        list_move(&sb->dirty_list,&root->ptrs[i]->dirty);
        root->ptrs[i]->parent = _new;
    }
...
static int __do_btree_split_child(struct stp_btree_info *sb,struct stp_bnode *parent,struct stp_bnode *child,int idx)
{
    ...
    printf("split_child:debug parent\n");
    sb->ops->debug(parent);
    printf("--------end(%s)------\n",__FUNCTION__);
